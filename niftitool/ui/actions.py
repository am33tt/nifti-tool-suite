...
marshal results back onto the Tk main thread via :meth:`self.after`.
"""
...
class ActionsMixin:
    ...
    def _load_path(self, path):
        if not HAS_NIBABEL:
            messagebox.showerror("Missing dependency", "nibabel is not installed.")
            return
    ...
    def _staged_load(self, path):
        try:
            # Stage 1: header only — no pixel data is read here.
            img = load_nifti(path)
            self.after(0, self._prog.advance)

            # Stage 2: wrap the proxy. This is essentially free; the file
            # is memory-mapped (.nii) or held as a lazy decoder (.nii.gz).
            # No full conversion to float32 — that used to dominate load.
            ...
            # Kick off the full materialisation in the background so that
            # subsequent slider drags and the 3-D view get a real RAM
            # array (no proxy decode per slice — especially important for
            # .nii.gz where every uncached slice would re-decompress).
            threading.Thread(
    ...
    def _materialise_full_volume_bg(self):
        """Promote the lazy proxy to a real float32 ndarray in RAM.

        Runs in a daemon thread.  Once the array is ready we swap it in
        on the Tk thread, rebind the slice cache, and notify the 3-D
        viewer so it can prepare its ``vtkImageData``.
        """
        try:
            g = self._gray
            if not isinstance(g, LazyGrayVolume):
                return
            arr = g.to_array()  # heavy step (RAM read or .nii.gz decode)

            def _swap():
                # Only swap if the same volume is still loaded — guard
                # against a new file having been opened mid-flight.
                if self._gray is g:
                    self._gray = arr
                    ...
                    # Let the 3-D tab know there's a real array to show.
    ...
    def _get_gray(self):
        """Return the gray volume as a plain numpy array (materialise if lazy).

        Use this only when the *whole* volume is needed (calibration,
        E-Map, full histogram).  The triplanar viewer should slice
        ``self._gray`` directly so the lazy proxy stays lazy.
        """
    ...
    def _require_img(self) -> bool:
        if self._img is None:
            messagebox.showwarning("No file", "Please open a NIfTI file first.")
            return False
        return True

    def _require_emap(self) -> bool:
        if self._E_map is None:
            messagebox.showwarning("No E-Map", "Please compute the E-Map first.")
            return False
        return True

    def _ask_save_path(self, default_name: str, ext=".nii.gz", ftypes=None):
        ftypes = ftypes or [("NIfTI gz", "*.nii.gz"),
                            ("NIfTI",    "*.nii"),
                            ("All",      "*.*")]
    ...
    def _update_stats(self):
        """Live stats for the left-hand panel.

        For very large volumes we subsample to
        :data:`config.STATS_SUBSAMPLE_VOXELS` so the panel update stays
        snappy; the histogram tab shows the full distribution.
        """
        if self._gray is None:
            return
        g = self._gray
        if isinstance(g, LazyGrayVolume):
            # Avoid pulling the whole file just for the live panel.
    ...
    def _do_rotate(self):
        ...
        try:
            ...
            messagebox.showerror("Bad input", "Angle must be a number.")
            return
        out_path = None
    ...
    def _do_crop(self):
        ...
        try:
            ...
            messagebox.showerror("Bad input", "Crop start/end must be integers.")
            return
        out_path = None
    ...
    def _do_calibrate(self):
        ...
        try:
            ...
            messagebox.showerror("Bad input", "All calibration fields must be numbers.")
            return
    ...
    def _do_compute_emap(self):
        ...
        try:
            ...
            messagebox.showerror("Bad parameters", str(ex))
            return
    ...
    def _export_labels_nifti(self):
        if self._labels is None:
            messagebox.showwarning("No labels", "Compute E-Map first.")
            return
        stem = self._path.name.replace('.nii.gz', '').replace('.nii', '')
    ...
    def _export_stats_csv(self):
        if not self._E_stats:
            messagebox.showwarning("No stats", "Compute E-Map first.")
            return
        stem = self._path.name.replace('.nii.gz', '').replace('.nii', '')
        out = self._ask_save_path(
